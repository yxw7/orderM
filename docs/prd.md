...
| 9a | 非连续出版物订单 | 订单行更换供应商 | P1 | 行内按套数迁出 / 工具栏按勾选行全量可迁出；生成待发订新单 |
...
| 30 | 采访验收 | 逐条收货-收/换/退处置弹窗 | P0 | 纵向合并一次确定；有草稿带入并显示「已带入预验收数据」；确认后清草稿 |
...
4. 行状态为已发订/处理中、验收为待验收/部分收货/换货中且可迁出套数 ≥ 1 的行，可从操作列按指定套数更换供应商；勾选多行且订户/资源类型/语种/采选方式一致时，可从工具栏批量全量迁出并生成待发订新订单。
5. 点击订单行号进入详情页。
...
- **查重范围**（位于查重字段下方；**仅当重复类型为「不限」或「馆藏查重」时显示**；选择「订单查重」时隐藏且不提交范围）
  - 标题旁标注：「（未选择时按不限范围查重）」
  - **所属分馆**（branchCodes）：可搜索多选；选项来自**四级馆藏地**中使用中的分馆；展示「分馆编码 | 分馆名称」（如 `ST001 | 首都图书馆`）；支持全选/反选（作用于当前过滤结果）；已选标签超出约三行高度时触发器内纵向滚动；可清空
  - **所属馆藏地**（collectionCodes）：可搜索多选；选项来自四级馆藏地中使用中的馆藏地；展示「馆藏地编码 | 馆藏地名称」（如 `ST01 | 哲学社会科学图书借阅室 (4204)`）；**随分馆联动**——已选分馆时仅展示这些分馆下馆藏地**并集**；**未选分馆时展示全部馆藏地**；变更分馆后剔除不再属于当前选项集的馆藏地；支持全选/反选与标签区滚动
  - 打开弹窗时两字段**默认空**；分馆与馆藏地均未选时按**不限范围**查重

###### 5.3.5.5.3 交互逻辑

- 选择重复类型后立即生效：切换为「订单查重」时隐藏查重范围；切换回「不限」或「馆藏查重」时重新显示（已选分馆/馆藏地保留）
- 若勾选「全部」，则选中全部字段；若取消「全部」，则全部取消；若字段勾选状态变化，则同步「全部」勾选状态
- 查重范围：支持关键字联想过滤选项；点选后下拉保持打开以便连续多选；未选分馆/馆藏地仍允许提交；仅「不限」「馆藏查重」提交所属分馆（branchCodes）、所属馆藏地（collectionCodes）数组，「订单查重」按空数组提交
...
- 其他字段比对时忽略大小写
- 查重范围未选所属分馆与所属馆藏地时视为不限；已选值随查重配置一并提交
...
- 右侧展示**当前单选书目**的 MARC；字段按语种过滤（规则同前）
- 不显示底部分页栏；直接点页签时使用当前单选书目（若无选中则回落第一条）

*单件页签*

- 左右分栏：左侧馆藏树**固定 20%**（不可拖拽调整），宽度不足时左侧出现横向滚动条；右「单件列表」
- 一 / 二 / 三级节点点击仅展开 / 收起；四级馆藏地或「未关联馆藏」点击后右侧过滤为对应所属馆藏地（或未分配）单件
- 进入页签或切换单选书目时未选四级叶子，右侧显示该书目**全部**单件
- 不显示底部分页栏；列：**馆藏状态**、条码号、索书号、所属馆、所属馆藏地、所在馆、所在馆藏地、借阅类型、卷册描述、**装帧**（精装/平装）、**排架标引分类**、**排架标引**、登到时间
...
- **订单查重**比对范围：当前登录馆员关联订户下的全部订单行（不含当前行自身）
- **馆藏查重**比对范围：馆藏书目库
- **馆藏重复判定**：匹配到馆藏书目即「有」，与单件数量是否为 0 无关
- **单件数量**：与该书目全部关联单件数一致；0 本时单件页签左侧「暂无馆藏分布」、右侧「暂无单件信息」
- **自动关联书目**：规则同前（查重返回时静默关联第一条；打开面板不再重复关联）
- 重复标识按本次查重配置的重复类型分别更新；「不限」时同时更新两列
- 馆藏书目表格字段、MARC 展示字段取自查重触发订单行所属订单的资源类型与语种
...
- 当前页签无数据时显示「暂无查重结果」或「暂无单件信息」/「暂无MARC信息」
- 若点击「有」但数据异常，则提示「查重结果数据不存在」
...
将订单行的**可迁出套数**迁出，生成一条待发订新订单（可同时指定新供应商、预算与撤订原因）；原行对应套数记入发/收/换/退/撤订（flowStats）的撤订段。提供两个入口：

1. **行内**：操作列「更换供应商」——用户填写迁出套数（1～可迁出），生成 1 条新订单 + 1 条新订单行。
2. **工具栏批量**：勾选多行后「更换供应商」——不填套数，每行按当前可迁出套数全量迁出，生成 1 条新订单 + N 条新订单行。
...
| 行内弹窗 | 标题「更换供应商」；确认「生成新订单」；订单名称上方只读展示当前行「发订套数：N  已收货套数：N  已换货套数：N  已退货套数：N」（取自发/收/换/退/撤订）；字段：订单名称、套数、供应商、预算名称、原因、备注 |
| 批量弹窗 | 标题「更换供应商」；确认「生成新订单」；字段同前但**无套数** |
| 订单名称（orderName） | 必填；最多 50 字符；打开时预填 `{原订单名称}-{n}`（见业务规则）；提交**不校验**名称是否重复 |
| 套数（仅行内） | 必填；打开时默认 = 当前可迁出套数；须为 1～可迁出的整数 |
| 供应商（supplier） | 必填；默认带入关联订单供应商；选项按采选方式（method）过滤 |
| 预算名称（budget） | 采选方式为交换或捐赠时可空且禁用选择；其余必填；默认带入关联订单预算 |
| 原因 | 必填；下拉取「退换撤订原因参数」中类型为撤订且使用中的原因；无可用原因时提示先去设置配置 |
| 备注 | 非必填；行内打开时默认带入原订单行备注；写入新订单备注类字段 |
| 成功确认弹窗 | 行内：「已生成新订单 {订单号}，原行对应套数已撤订，是否立即查看？」；批量：「已生成新订单 {订单号}，订单行种数 {种}，套数 {套}，册数 {册}，是否立即查看？」；取消 / 确定 |

###### 5.3.5.12.3 交互逻辑

**行内**

1. 满足显示条件时操作列出现「更换供应商」；点击打开行内弹窗并预填表单。
2. 用户可改订单名称、套数、供应商、预算、原因、备注；必填项在字段下方即时校验提示。
3. 点击「生成新订单」：校验表单与可迁出套数 → 创建待发订新订单及 1 条新订单行 → 回写原行撤订段 → 关闭弹窗 → 弹出成功确认。
4. 成功确认：取消仅关闭；确定则切换至「订单列表」页签。

**工具栏批量**

1. 未满足启用条件时工具栏按钮置灰。
2. 启用后点击打开批量弹窗（无套数），预填首行关联订单的订单名称建议值、供应商、预算。
3. 点击「生成新订单」：校验表单、勾选范围一致性及每行可迁出 ≥ 1 → 创建 1 条待发订新订单及 N 条新订单行 → 回写各原行撤订段 → 关闭弹窗 → 弹出带种数/套数/册数的成功确认。
4. 成功确认：取消仅关闭；确定则切换至「订单列表」页签。
...
| 覆盖 | 订单名称、供应商、预算、备注、原因取自弹窗 |
| 新订单行（行内） | 恰好 1 行；套数与发订段 = 填写套数；行状态待发订；是否催缺为「否」；书目/资源字段自原行复制 |
...
- **后置**：存在待发订新订单及对应新订单行；原行已按迁出套数记撤订
...
| 未填订单名称 / 未选供应商 / 应填未选预算 / 未选原因 | 字段下方红色提示 |
...
- 表头无效红色提示；行号变更未再读琥珀色提示。  
- 列映射表：文件列名｜系统标准字段｜是否校验。  

**交互**

- 「读取表头」：按 Excel **物理行号**生成列名（无原始预览）。首次可读出默认列映射并套用默认是否校验；已有映射尽量保留。  
- 底栏：「取消」｜「上一步」｜「下一步」。下一步不要求必填已映完；点「下一步」时才校验必填映射，未齐提示并停留。点「下一步」**不**静默写模板。  
...
| 选用某一模板 | 若模板含表头行号（headerRow），写入表头行号输入框；按该行号重新读取表头（生成列名）；将模板列映射（columnMapping）按当前文件列名对齐写入；标准字段按当前验收单资源类型×语种清洗非法项；恢复是否校验（mustMatchFields，缺省则用默认勾选集） |
| 切回「选择模板」 | 清空列映射与是否校验（列名仍保留时各列置为不映射、不勾选）；**不**清空表头行号与已读列名 |
| 「保存模板」（未选模板） | 须已成功读取表头，否则提示「请先读取表头后再保存模板」。弹出「请输入模板名称」；取消则中止；名为空提示「请输入模板名称」；名称已存在则确认「模板「名称」已存在，是否覆盖？」；保存成功提示「模板已保存」，下拉选中新建名称 |
| 「保存模板」（已选模板） | 须已成功读取表头。直接覆盖当前选中模板的表头行号 + 列映射 + 是否校验列表；提示「模板已更新」（不再弹名称框） |
| 「删除模板」 | 未选模板时按钮禁用。已选时确认「确定删除模板「名称」？」；确定后删除并清空下拉为「选择模板」；**不**自动清空当前列映射（仅去掉选中关系） |

写入模板的内容：表头行号（headerRow）+ 列映射（columnMapping）+ 是否校验字段列表（mustMatchFields，标准字段 key）。  
...
- 订单行号、收货套数不可勾是否校验。  
- 默认勾选是否校验：实洋、套内册数/件数、定价（纸质）/码洋（视听）。  
...
- 进度：「正在校验，请稍候…」+ 进度条。  
- 完成：成功/失败条数汇总；橙色提示只导入成功行；失败时可「下载失败数据」。  

**交互**

- 进入后自动校验；底栏「取消」｜「下一步」（成功条数=0 时置灰）。  
- 下载失败数据：仅失败行；原发货单列（原顺序）+ 失败原因。  

**校验规则**

匹配池：当前馆员关联订户范围内的可收货订单行，关联订户下可收货订单行，且与验收单资源类型、语种（中/外）、采选方式、供应商一致。
...
| **不一致** | 仅对用户勾选「是否校验」且已映射的标准字段，与订单行比对不等（收货套数不走本类，仅走套数非法） | 「"{字段}"订单行 …/发货单…」 |
| **通过** | 未命中以上失败类型；套数满足 **0＜收货套数≤待收** | — |

- 允许部分成功：仅「通过」行可进入第 4 步入库。  
...
- 第 3 步「下一步」进入并自动写通过行草稿；进度中不可关；完成后仅「关闭」。不可回上一步。  

**草稿规则**

- 维度：验收单号（acceptanceId）+ 订单行号（orderLine）；内容为通过行已映射值（含未勾选是否校验的差异字段）。  
...
合并处置弹窗纵向展示收货、换货、退货三段；内容区可上下滚动；底栏统一「确定」，一次提交已填处置。弹窗固定高度，顶栏 / 统计条 / 底栏固定。
...
3. 收、换、退均无有效数量 → 提示「请至少填写一种处置套数」，不写库、不关弹窗。
4. 校验通过后按 **收货 → 换货 → 退货** 依次写库；成功提示「提交成功」，刷新列表与头信息后**一律关闭弹窗**（不按待收是否为 0 决定）。
5. 点遮罩或关闭不提交草稿。
...
| 场景 | 处理 |
|------|------|
| 三种处置均无有效数量 | 「请至少填写一种处置套数」；不关弹窗 |
| 收货缺必填 | 提示填写对应字段；不关弹窗 |
| 换/退有数量缺原因 | 提示选择换货/退货原因 |
| 合计超待收 | 「处置套数合计不能大于待收套数（当前待收 N）」；不写库、不关弹窗 |
