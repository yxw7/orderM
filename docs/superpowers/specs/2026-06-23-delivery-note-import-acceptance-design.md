...
**组件**：`DeliveryImportPickOrderLineModal.vue`（预览 / 任务详情内弹窗）
...
| 标题栏 | 「选择订单行」+ 关闭 |
| 检索区 | 检索字段下拉 + 关键词输入 + 「检索」 |
| 列表区 | 可横向滚动的订单行表格；左侧复选框多选 |
| 底栏 | 取消、确定（至少选 1 条） |

弹窗宽度需容纳多列（建议 `max-w-7xl` 或等效全宽约束）。

**检索字段**（下拉单选 + 关键词，对展示值做包含匹配）：
...
| 订单号 | 由订单行号解析，见下列「订单号」 |
| 正题名 | |
| 资源标识 | 对 ISBN / ISRC / 商品条码 / 目录号拼接结果检索 |
| 馆址 | |

**列表列**（纸质书与视听资料**统一列集**，列顺序如下；对齐 PRD 关联订单行列表 §5.5.5.1，不含序号、结算状态、发订时间）：

| 列 | 字段 / 展示规则 |
|----|----------------|
| 订单号 | 从订单行号解析：`st00120250921005-3` → `st00120250921005`（末段 `-` 之前） |
| 馆址 | `location` |
| 订单行号 | `orderLine` |
| 正题名 | `title`（视听资料列名仍为「正题名」） |
| 资源标识 | `isbn` / `isrc` / `barcode` / `catalogNo` 非空项以 `;` 拼接；皆空则 — |
| 载体 | `carrier`；无则 — |
| 作者 | `author` |
| 出版社 | `publisher`；无则 — |
| 出版时间 | `pubYear`（或 `publishYear` / `publishTime`）；无则 — |
| 定价 | `price` 或 `actualPrice` |
| 币种 | `currency` |
| 套内册数 | `copies`（视听资料套内件数同源字段，列名统一为「套内册数」） |
| 套数 | `orderedSets` |
| 行状态 | 默认「已发订」 |
| 验收状态 | 有 `acceptanceStatus` 则用原值；否则待收 > 0 为「待收货」，否则「收货完成」 |
| 发/收/换/退/撤订 | `counts` 五段，如 `2/0/0/0/0` |

**格式化逻辑**：`data/delivery-import.js` — `PICK_ORDER_LINE_COLUMNS`、`formatPickOrderLineDisplayRow`、`filterPickOrderLineRows`。

---

## 8. 交互流程与向导步骤

> **架构说明（2026-07-10）**：创建阶段已迁移至异步任务架构，见 [2026-07-10-delivery-import-async-task-design.md](./2026-07-10-delivery-import-async-task-design.md)。本节 §8.2 的 **5 步全页向导** 为初版设计；当前原型为 **验收单管理页内弹窗 4 步**（①～③ 配置 + ④ 创建任务），匹配预览与提交在 **导入任务 → 任务详情** 中续办。本节 §9 预览结构仍适用于任务详情预览核对页。
...
> **实现位置（2026-07-10）**：预览核对在 **导入任务详情页** 中展示。Tab 过滤、分批提交、防抖自动保存、已提交只读等见 [2026-07-10-delivery-import-async-task-design.md §9](./2026-07-10-delivery-import-async-task-design.md)。**上下主从 + 订单行卡片** 见 [2026-07-10-delivery-import-preview-master-detail-design.md](./2026-07-10-delivery-import-preview-master-detail-design.md)。主表已映射列按任务快照 `mappingConfig` + `fileColumns` 顺序（`getPreviewMappedColumns`）。
...
> **2026-07-10 修订**：行内展开子表 **废止**。订单行在任务详情 **下区卡片** 中展示，详见 [2026-07-10-delivery-import-preview-master-detail-design.md §7](./2026-07-10-delivery-import-preview-master-detail-design.md)。

卡片内信息区为 **16 列**（同 §7.1 选行弹窗）；处置区为 **单列** 纵向表单：
...
下区顶栏：**添加订单行**（弹窗追加）、**删除订单行**（卡片勾选后批量删除）。上区发货主行操作列 **仅删除**。
...
| 页面/组件 | 路径 / 说明 |
|-----------|-------------|
| 验收单管理 | `AcceptanceManageView.vue` — 「导入发货单」按钮（默认置灰；列表勾选一条可导入验收单后启用）→ 打开 **创建弹窗** |
| 导入发货单创建弹窗 | `DeliveryImportCreateModal.vue` — 弹窗 4 步（①～④）；详见异步任务 spec §5 |
| 导入任务列表 | `DeliveryImportTaskListView.vue` — 模块级任务列表；操作列按状态区分 |
| 导入任务详情 | `DeliveryImportTaskDetailView.vue` — 匹配进度 / 预览核对；详见异步任务 spec §8～§9 |
| 选择订单行弹窗 | `DeliveryImportPickOrderLineModal.vue` — 预览内弹窗；检索并多选订单行；列表列见 §7.1 |
| 业务逻辑 | `data/delivery-import.js` — 字段池、匹配引擎、馆址拆分、校验、映射模板、预览列 helper |
| 任务逻辑 | `data/delivery-import-tasks.js` — 任务 CRUD、异步匹配、草稿、提交 |
| 上下文传递 | `stores/acceptance.js` — 列表勾选验收单写入 store / localStorage |

> 旧全页向导 `DeliveryImportView.vue`（5 步同步流程）保留于代码库，路由已重定向至导入任务列表，**不再作为主流程**。
...
12. 选择订单行弹窗：列表展示 §7.1 所列 16 列；检索支持订单行号、订单号、正题名、资源标识、馆址
13. 收货 + 换货 + 退货同一子行：三者之和 ≤ 待收套数；主行各子行处置合计 ≤ 发货套数
14. 换货：套数>0 须选原因
15. 退货：套数>0 须选原因
16. 列映射模板：切换即应用；勾选保存并在下一步时写入；同供应商+类型+语种可存多个命名模板
17. 第 ② 步下一步：校验映射并初始化默认匹配字段
...
| 流程架构 | **异步任务 + 创建弹窗**（见 2026-07-10 spec） | 2026-07-10 架构迁移 |
| 预览续办 | 导入任务列表 → 任务详情；防抖自动保存 | 2026-07-10 架构迁移 |
