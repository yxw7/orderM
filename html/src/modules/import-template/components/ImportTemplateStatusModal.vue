<template>
  ...
  >
    <div class="flex items-start gap-3">
      ...
      <input v-model="form.name" type="text" placeholder="请输入" readonly
    ...
    <div class="flex items-start gap-3">
      ...
      <input v-model="form.code" type="text" placeholder="请输入" readonly
        class="flex-1 border border-gray-200 rounded px-3 py-2 text-sm bg-gray-50 text-gray-600 cursor-not-allowed">
    </div>
    <div class="flex items-start gap-3">
...
watch(() => props.open, open => {
  if (open && props.row) {
    form.value = {
      name: props.row.templateName,
      code: `${props.row.resourceType} / ${props.row.language}`,
      remark: ''
    };
  }
});
