...
/**
 * 按订单号分组生成催缺单，自动过滤已收货/已退货书目
 * @param {Object[]} rows - 勾选的订单行
 * @returns {Object[]}
 */
export function generateShortageOrdersByOrderId(rows) {
  const eligibleRows = rows.filter(row => canGenerateShortage(row.acceptanceStatus));
  const groups = {};
...
export function buildShortageListRows(groups, orders, startId = 1) {
  ...
  return groups.map((group, index) => {
    ...
    const sets = group.lineRows.reduce((sum, row) => sum + (Number(row.sets) || 0), 0);
    return {
      id: startId + index,
