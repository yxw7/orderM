...
| 8 | 非连续出版物订单 | 订单行列表-查重 | P0 | 馆藏+订单查重；批量≤50；空书目记录号自动关联；结果顶部下拉面板 |
...
| 9a | 非连续出版物订单 | 订单行更换供应商 | P1 | 未收货迁出生成待发订新单；排除已催缺行 |
...
| 19 | 书目查询 | 加入订单弹窗 | P0 | 馆员关联订户校验、待发订候选过滤、检索精确匹配、币种取自货币信息、三区结构多选与馆址套数 |
...
| 26 | 采访验收 | 验收单管理-预验收 | P0 | 须勾选一条未开始/进行中验收单；三步向导上传→映射→校验；不收货 |
| 27 | 采访验收 | 验收单管理-批验收 | P0 | 四步向导：上传→映射→校验→确认验收；收货用发货单数据 |
| 28 | 采访验收 | 验收单管理-申请结算/导出 | P1 | 批量申请结算；导出配置与导出清单 |
| 29 | 采访验收 | 逐条收货-头信息与检索列表 | P0 | 当前工作验收单、检索、选行打开处置 |
| 30 | 采访验收 | 逐条收货-收/换/退处置弹窗 | P0 | 页签合并处置；待收=发订−已收−已退；条码预览自动关闭 |
...
**实现文件**：`OrderLineListPanel.vue`、`OrderLineSearchPanel.vue`、`OrderLineChangeSupplierModal.vue`、`ActualBibRecordNoMarker.vue`、`CatalogRecordFloatingWindow.vue`、`DedupConfigModal.vue`、`DedupResultDrawer.vue`
...
4. 同一订单下、未生成催缺、且验收为待验收/部分收货/换货中的行，可将未收货套数更换供应商并生成待发订新订单。
5. 点击订单行号进入详情页。
...
- 启用条件：同时满足以下条件时按钮高亮可点击：
  1. 至少勾选一条订单行
  2. 所勾选订单行数量**不超过 50 条**
  3. 所勾选订单行均为**待发订**（若所属订单状态为待发订，则其下订单行均视为待发订）
  4. 所勾选订单行属于**相同资源类型**（取自所属订单：纸质书 / 视听资料）
  5. 所勾选订单行属于**相同语种分类**（取自所属订单：中文 / 外文）
- 点击后：打开查重配置弹窗

**单个查重文字链**

- 位置：订单行列表操作列
- 显示条件：仅当该行可查重时显示（行状态为待发订，或所属订单状态为待发订）
- 非待发订行：不显示查重文字链
...
- 若订单行行状态不为待发订，且所属订单状态亦不为待发订，则不可查重
- 若所属订单状态为待发订，则其下订单行均视为可查重
- 若批量勾选数量大于 50 条，则批量「查重」按钮置灰，不可打开查重配置
- 若批量勾选行的资源类型或语种分类不一致，则批量「查重」按钮置灰
- 单行查重不受批量勾选数量与「同资源类型 / 同语种」限制，但目标行须满足可查重条件
...
点击查重入口后弹出查重配置弹窗，用户选择重复类型、查重字段与查重范围后执行查重。
...
- **查重范围**（位于查重字段下方；**仅当重复类型为「不限」或「馆藏查重」时显示**；选择「订单查重」时隐藏且不提交范围）
  - 标题旁标注：「（未选择时按不限范围查重）」
  - 勾选系统预设的**分馆编码前缀通配符**；每项展示「通配符 | 馆名」（如 `ST* | 首都图书馆`）
  - 支持多选；命中规则：分馆编码（branchCode）以通配符去掉 `*` 后的前缀开头（忽略大小写）；多模式取**并集**
  - 选项列表固定高度，超出时纵向滚动；**不支持**关键字搜索与自由输入
  - 打开弹窗时**默认不选**；未选任何通配符时按**不限范围**查重
  - 预设清单（展示顺序固定，`ST*` 置顶）：

| 通配符（branchPatterns） | 展示文案 |
|--------------------------|----------|
| ST* | ST* \| 首都图书馆 |
| CP* | CP* \| 昌平区图书馆 |
| CY* | CY* \| 朝阳区图书馆 |
...
说明：`DC*`/`CW*` 同属东城区、`XC*`/`XW*` 同属西城区，作为两条独立预设保留。

###### 5.3.5.5.3 交互逻辑

- 选择重复类型后立即生效：切换为「订单查重」时隐藏查重范围；切换回「不限」或「馆藏查重」时重新显示（已选通配符保留）
- 若勾选「全部」，则选中全部字段；若取消「全部」，则全部取消；若字段勾选状态变化，则同步「全部」勾选状态
- 查重范围：仅可勾选上表预设通配符；未选通配符仍允许提交；仅「不限」「馆藏查重」提交查重范围通配符（branchPatterns），「订单查重」按空范围提交
...
- 其他字段比对时忽略大小写
- 查重范围未选任何通配符时视为不限；已选通配符以查重范围通配符（branchPatterns）一并提交（如 `ST*`、`PG*`）
- 若某已选通配符未匹配到任何使用中分馆，仍允许提交，不弹错
...
- 从页面**顶部向下滑出**，全宽展示；默认高度约视口 **75%**（上限不超过视口减边距）；馆藏查重切换至「MARC信息」「单件」时增高至约 **88%**，以提升小分辨率下内容区高度
- 面板结构自上而下：**标题栏** → **摘要信息区** → **页签栏**（仅馆藏查重）→ **结果内容区（可滚动）** → **分页栏**（书目页签 / 订单查重；MARC / 单件页签不占位）
- 标题：馆藏查重时显示「**馆藏查重结果**」；订单查重时显示「**订单查重结果**」
- 摘要区**一行横向展示**：订单行号（orderLineNo）、查重字段（本次所用字段中文名，顿号分隔）、重复记录数；查重字段过长时截断，悬浮可查看完整内容
- 摘要区**不再**提供订单行书目明细的展开/收起；订单行信息改在馆藏查重「MARC信息」页签左侧展示
- 结果数据较多时，中间结果内容区出现**垂直滚动条**；标题栏、摘要区、页签栏、底部分页栏固定不滚动
- 点击遮罩或右上角 × 关闭面板

**馆藏查重结果**

「书目」「**MARC信息**」「**单件**」页签固定在摘要区下方、滚动区外，默认展示「书目」页签。切换页签时结果内容区**滚动位置重置为顶部**；切换至 MARC / 单件时面板略增高且不占位底部分页栏，以扩大内容区。

**馆藏查重：有单件与无单件（重点说明）**

馆藏查重比对**馆藏书目库**。若匹配到书目记录，则列表馆藏重复（holdingDuplicate）显示「有」，**与单件数量是否为 0 无关**。馆员既需确认「书目著录是否已存在」，也需通过「单件数量」判断「是否有实体馆藏」。
...
| 「单件」页签 | 展示该书目关联的单件明细，行数与单件数量一致 | 展示「暂无单件信息」 |
| MARC / 关联操作 | 可用 | 可用 |

**单件数量**统计规则：单件数量与「单件」页签明细行数一致（已分配馆藏地单件 + 未关联馆藏单件）。未分配馆藏地的单件归入馆藏树「**未关联馆藏**」节点，以与馆藏地一致的蓝色圆角徽章「N本」展示。

*书目页签*

- 以**表格**形式展示重复书目，每条书目占一行
- 列顺序：**展开/收起** → **单件数量** → 书目字段（随资源类型与语种动态切换）→ **操作**
- 书目展示字段根据**当前查重订单行所属订单的资源类型与语种**动态切换：
...
- **单件数量**列：圆角徽章「N本」；数量 > 0 为蓝色，数量 = 0 为灰色；数值与「单件」页签明细行数一致
- 列较多时表格支持**横向滚动**；纵向滚动时表头吸顶固定
- 点击展开图标可展开/收起当前书目行
- 若该书目有单件，则展开后展示馆藏树；若无单件，则展开后显示「暂无馆藏分布」
- 馆藏树分支节点（机构/馆区/分馆）支持展开/收起；馆藏地及「未关联馆藏」叶子节点以蓝色圆角徽章展示单件数，如「1本」「2本」
- 默认**折叠**馆藏树；用户点击展开图标后展示馆藏分布
- 操作列：
  - 「**查看**」：切换至「MARC信息」页签，展示当前书目的 MARC 详细信息（不跳转外部页面）
  - 「**单件**」：切换至「单件」页签，展示当前书目关联的单件明细
  - 「**关联**」/「**取消关联**」：写入或清空当前查重订单行的书目记录号（bibRecordNo）
    - 若当前订单行已有书目记录号（bibRecordNo）且与该行书目记录号相同，则显示「**取消关联**」；其余行显示「**关联**」
    - 若点击「**关联**」，则将该书目行的书目记录号写入订单行
    - 若点击「**取消关联**」，则清空订单行的书目记录号（bibRecordNo）
    - 关联/取消关联后，订单行列表「书目记录号」列同步更新

*MARC信息页签*

- **左右分栏**（默认各 **50%** 宽度，中间分隔条可左右拖拽调整，左侧约 20%–80%）：左侧「订单行信息」，右侧「MARC信息」
- **左侧**：展示当前查重订单行的书目明细字段（按所属订单资源类型 / 语种，规则与订单行详情书目信息一致）；字段为空显示「—」；过高时可独立纵向滚动
  - **纸质书 · 中文**：正题名、ISBN、副题名、分卷号、分卷名、分类号、出版社、作者、出版年、定价、版本、丛编、主题词、读者对象、装帧形式、尺寸、正文语种、卷数、出版地、一般性附注、图书简介、备注
  - **纸质书 · 外文**：ISBN、学科大类、学科细分、中图分类号、中译名、题名、副题名、责任者、丛编、出版社、装帧形式、出版日期、版次、页数、币种、价格、主题词、读者对象、尺寸、语种、简介、精简装ISBN对照、馆藏信息、审读级别、获奖信息、目次信息、分卷号、分卷名、作者简介、书评、备注
  - **视听资料 · 中文**：ISBN、ISRC、题名、载体、出版社、版本/格式、著者、币种、码洋、彩胶颜色、限量编号、厂牌、系列名称、是否签名、是否老唱片、获奖信息、北京出版社、分类、盘号、老唱片品牌、剧种、年代、备注
  - **视听资料 · 外文**：ISRC、题名、载体、商品条码、目录号、外文原文题名、出版方、码洋、币种、备注、厂牌
- **右侧**：表格展示当前选中馆藏书目的 MARC 字段详情，列：**字段名**、**指示符**（表头不换行）、**字段内容**；表头固定，内容区可独立纵向滚动
- 展示字段根据**当前查重订单行所属订单的语种**动态过滤：
...
- 表格展示当前选中书目关联的实体单件明细，列：**条码号**、**索书号**、**所属馆**、**所属馆藏地**、**所在馆**、**所在馆藏地**、**借阅类型**、**卷册描述**、**登到时间**
- 字段为空时单元格显示「—」；列较多时支持横向滚动；纵向滚动时表头吸顶
- **单件页签不显示底部分页栏**（展示当前书目全部关联单件）
- 直接点击「单件」页签：默认展示查重结果**第一条书目**关联的单件信息
- 在书目页签点击某条「单件」：切换至本页签并展示该书目关联单件
- 单件明细行数与书目列表「单件数量」一致；若无单件，展示「暂无单件信息」

**订单查重结果**

- 以表格形式展示：订单行号（orderLineNo）、馆址（site）、正题名（title）、作者（author）、出版社（publisher）、出版时间（publishTime）、定价（price）、币种（currency）、套内册数（copiesInSet）、套数（sets）、行状态（lineStatus）、发订时间（issueTime）
- 列较多时表格支持**横向滚动**；纵向滚动时表头吸顶固定
- 暂无操作列
...
- **馆藏查重 · 书目页签**：默认 **5** 条/页，可选 **5 / 10 / 20 / 50** 条/页
- **订单查重结果**：默认 **50** 条/页，可选 **50 / 100 / 200** 条/页
- 馆藏查重 **MARC 信息 / 单件** 页签不展示分页栏
- 打开面板时按查重类型重置为第 1 页及对应默认每页条数
- 无重复记录时，内容区显示「暂无查重结果」

###### 5.3.5.6.3 交互逻辑

- 若标识为「有」，则可点击打开对应类型（馆藏/订单）的查重结果面板；若为「无」或空白，则不响应点击
- 馆藏结果中，书目行展开/收起独立控制；馆藏树各层级节点亦可独立展开/收起
- 若点击「查看」，则切换至 MARC 页签并展示对应书目 MARC 信息，内容区滚回顶部
- 若点击「单件」，则切换至单件页签并展示对应书目关联单件，内容区滚回顶部
- 若点击「关联」，则将当前书目记录号（bibRecordNo）写入查重订单行；若点击「取消关联」，则清空订单行书目记录号（bibRecordNo）
- MARC / 单件页签默认展示第一条书目信息
- 分页切换时保持当前页签状态；打开面板时重置为第 1 页、书目页签，馆藏树默认全部折叠（馆藏查重每页条数重置为 5，订单查重重置为 50）
- 打开结果面板时**不再**执行自动关联（自动关联仅在查重返回落库时处理）

###### 5.3.5.6.4 业务规则

- **订单查重**比对范围：当前登录馆员**关联订户**下的全部订单行（不含当前行自身）
- **馆藏查重**比对范围：馆藏书目库
- **馆藏重复判定**：若匹配到馆藏书目记录，则列表馆藏重复（holdingDuplicate）为「有」，**与单件数量是否为 0 无关**
- **单件数量**：与「单件」页签明细行数一致（已分配馆藏地单件 + 未关联馆藏单件）；若为 0，则书目页签展示灰色「0本」，展开区展示「暂无馆藏分布」，单件页签展示「暂无单件信息」
- **有单件**：若单件数量 > 0，则展示馆藏树与单件明细；未分配馆藏地的单件归入「未关联馆藏」节点
- **无单件**：若单件数量 = 0，则书目仍出现在查重结果中，可查看 MARC、执行关联操作；单件页签为空
- **自动关联书目**：
  - 若本轮执行了馆藏查重（重复类型为「馆藏查重」或「不限」），且订单行书目记录号（bibRecordNo）去空格后为空，且本行馆藏查重结果至少一条且第一条书目记录号有效，则静默将结果**第一条**的书目记录号写入该订单行
  - 若书目记录号（bibRecordNo）已有值，则不覆盖
  - 若仅执行订单查重，或馆藏结果为空，或不满足上述条件，则不自动关联
  - 批量查重时逐行独立判断与写入
  - 「第一条」与结果面板书目列表顺序一致
  - 打开结果面板时不再重复自动关联；手动「关联 / 取消关联」规则不变
  - 若用户取消关联后再执行馆藏查重且结果非空，则因书目记录号为空将再次自动关联第一条
- 重复标识按本次查重配置的重复类型分别更新（若选择「不限」，则同时更新两列）
- 若重复标识为「有」，方可打开结果面板
- 馆藏书目表格字段、MARC 展示字段取自查重触发订单行所属订单的资源类型与语种，规则与上文一致

###### 5.3.5.6.5 前置/后置条件

- **前置**：已执行查重操作，列表标识列已更新
- **后置**：查重结果面板展示详细查重结果；列表书目记录号（bibRecordNo）已按自动关联规则刷新（若适用）

###### 5.3.5.6.6 异常处理

- 重复标识为「无」或未查重时，不响应「有」链接点击
- 结果为空时显示「暂无查重结果」，分页显示第 1/1 页
- 若点击「有」但后端数据异常（如重复记录已删除），则提示「查重结果数据不存在」
- 书目展开后若无馆藏单件（单件数量为 0），则展示「暂无馆藏分布」
- 单件页签无关联单件时，展示「暂无单件信息」
...
将勾选订单行的**未收货套数**迁出，生成一条待发订新订单（可同时指定新供应商与预算）；原行未收货部分按固定原因「更换供应商」记入发/收/换/退/撤订（flowStats）的撤订段。已生成催缺的订单行不可走本入口（应在催缺管理处理）。

###### 5.3.5.12.2 页面要素

| 要素 | 位置 / 规则 |
|------|-------------|
| 更换供应商按钮 | 订单行列表工具栏；无行内操作入口 |
| 更换供应商弹窗 | 标题「更换供应商」；确认文案「生成新订单」 |
| 订单名称（orderName） | 必填；打开时预填为原订单名称 +「-1」，可改；长度不超过订单名称上限 |
| 供应商（supplier） | 必填；下拉选项按原订单采选方式（method）过滤 |
| 预算名称（budget） | 若采选方式为交换或捐赠则可空且可不选；其余必填 |
| 备注 | 非必填；写入新订单备注类字段 |
| 成功确认弹窗 | 文案：「已生成新订单 {订单号}，原行未收货已按更换供应商撤订，是否立即查看？」；取消 / 确定 |

###### 5.3.5.12.3 交互逻辑

1. 未满足启用条件时按钮置灰；若点击则提示首条不满足原因。
2. 打开弹窗，预填订单名称，用户选择供应商、预算（按需）并可填备注。
3. 点击「生成新订单」：校验表单与未收货套数 → 创建待发订新订单及新订单行 → 回写原行撤订段 → 关闭弹窗并清空勾选。
4. 成功确认：取消仅关闭；确定则切换至「订单列表」页签。
...
1. 至少勾选 1 行
2. 所选行订单号（orderId）相同
3. 每行行状态（lineStatus）为「已发订」或「处理中」
4. 每行验收状态（acceptanceStatus）为「待验收」「部分收货」或「换货中」
5. 每行是否催缺（isShortage）不为「是」
...
- 已换套数不算未收货，不迁出
- 若勾选行中全部未收货套数均为 0，则禁止生成并提示「所选订单行没有可迁出的未收货套数」
- 若同批中部分行为 0：跳过这些行（不迁出、不改原行），仅处理未收货 > 0 的行

**新订单**

| 项 | 规则 |
|----|------|
| 数量 | 因限制同一订单号，始终生成恰好 1 条新订单 |
| 状态 | 待发订 |
| 继承 | 订户、馆址、资源类型、语种、采选方式、折扣等取自原订单抬头 |
| 覆盖 | 订单名称、供应商、预算、备注取自弹窗 |
| 新订单行 | 每个未收货 > 0 的原行对应一行；套数与发订段 = 该行未收货套数；行状态待发订；是否催缺为「否」；书目/资源字段自原行复制 |

**原行回写（不打开撤订原因选择弹窗）**

- 撤订段累加本次未收货套数；撤订原因固定为「更换供应商」
- 若发订套数全部进入撤订（无已收/已换/已退）→ 行状态变为「已撤订」
- 若仍保留已收等数量 → 保留订单行，并按更新后的发/收/换/退/撤订重算行状态与验收状态（例如已无待收则趋于「收货完成」/「已关闭」）

**与催缺更换供应商的边界**

| 入口 | 适用行 | 迁出/撤订数量口径 |
|------|--------|-------------------|
| 订单行列表「更换供应商」（本节） | 是否催缺 ≠「是」 | 未收货 = 发订 − 已收 − 已换 − 已退 |
...
- **前置**：勾选满足启用条件的订单行；原订单抬头可解析
- **后置**：存在待发订新订单及对应新订单行；原行未收货已记撤订；勾选已清空

###### 5.3.5.12.6 异常处理

| 场景 | 处理 |
|------|------|
| 跨订单号 / 行状态或验收不符 / 已催缺 | 按钮禁用或提示原因 |
| 全部未收货 = 0 | 提示「所选订单行没有可迁出的未收货套数」 |
| 未填订单名称 / 未选供应商 / 应填未选预算 | 提示请填写或选择对应字段 |
| 未找到原订单 | 提示无法生成 |
...
- 总种数取当前按种明细行数。  
- 总册数、总码洋、总实洋按资源类型展示（纸质书 / 视听资料分别对应页面汇总值；原型阶段与实现一致为按类型固定展示）。  
...
| 页签 | 资源类型 | 条件摘要 |
|------|----------|----------|
| 按种 | 纸质书 | 检索项（ISBN/订单行号/正题名）+ 关键词、验收人；展开开始/结束日期 |
| 按种 | 视听资料 | 检索项（ISBN/ISRC/题名/订单行号/载体）+ 关键词、验收人；展开日期 |
| 按册 | 纸质书 | 检索项（条码号/ISBN/正题名）+ 关键词、收货人；展开日期 |
| 按册 | 视听资料 | 检索项（条码号/ISBN/ISRC/题名/载体）+ 关键词、收货人；展开日期 |
...
**路由**：`#/acceptance`  
**实现文件**：`AcceptanceManageView.vue`、`AcceptanceFormModal.vue`、`PreAcceptWizardModal.vue`、`BatchImportWizardModal.vue`、`AcceptanceSettlementModal.vue`、`AcceptanceExportConfigModal.vue`  
**PRD 面板**：`html/src/prd/acceptance-manage-prd-data.js`（pageId：`acceptance-manage`）

#### 5.7.1 功能说明

管理验收单列表：检索、新增/编辑、设为当前工作验收单、结束/删除；提供预验收（差异核对）与批验收（映射后收货）向导；支持申请结算与列表导出。
...
3. 有关联订户且勾选一条未开始/进行中验收单时，可执行预验收或批验收。  
4. 勾选单据申请结算；按需配置导出字段并导出清单。  
...
三步向导核对发货单与可收货订单行差异；**不执行收货**。

###### 5.7.5.4.3 交互逻辑

1. 上传 → 2. 字段映射 → 3. 数据校验（完成）。  
- 入口须有关联订户；否则「您没有关联订户，无操作权限」。  
- 须勾选恰好 1 条未开始或进行中的验收单；否则按钮禁用或提示「请先在列表中勾选一条未开始或进行中的验收单」。打开前设为当前工作验收单。  
- 文件类型：xls、xlsx。  
...
- 提示：「将文件列映射到系统标准字段（标*字段为必填）」。  
- 必填：订单行号（orderLine）、码洋（listPrice）、实洋（netPrice）、收货套数（receiveQty）、套内册数（volCount）。  
- 可选字段按所选验收单资源类型（resourceType / type）仅含纸质书或视听资料字段（不同时提供两类）；下拉扁平展示，无分组标签。  
- 映射模板与批验收**共用**同一套已保存模板；应用模板时清除当前资源类型不允许的映射值。  
- 五字段未齐不可下一步。  

**校验（预验收）**

- 匹配范围：当前馆员关联订户下可收货订单行，且须与所选验收单的资源类型（type）、语种（lang）、采选方式（method）、供应商（supplier）一致。  
- 同行多种失败类型用「；」拼接；汇总仅计成功/失败行数。  
- 数据错误：订单行号为空或码洋/实洋/套数/册数无法解析 → 「数据解析失败」。  
- 验收单不匹配：订单行存在但头属性不一致 → 分项原因（如「供应商不一致（验收单 湖北三新/订单行 北京人天）」）；语种按中文/外文分档比对。  
- 未匹配：无对应可收货订单行。  
- 套数非法：收货套数 ≤ 0 → 「套数 ≤ 0」；＞待收 → 「套数大于待收」。  
- 不一致：已映射字段（除订单行号）与订单行不等（含 0＜收货套数＜待收）；若收货套数已记套数非法则不再因收货套数记不一致。  
- 金额两位小数比对；数量数值比对。  

**下载校验结果**：导出 Excel（.xls）= 原发货单全部列（原顺序）+ 校验结果 + 失败原因。

###### 5.7.5.4.6 异常处理

- 非 xls/xlsx：「请上传 xls/xlsx 格式文件」。  
- 无关联订户（解析）：「您没有关联订户，无法查看数据」。  
- 无可收货订单行：提示无法演示预验收。  

##### 5.7.5.5 功能模块名称：批验收向导

###### 5.7.5.5.1 功能描述

四步向导：上传 → 映射 → 校验 → 确认验收；通过行按**发货单已映射值**收货，不因字段不一致阻断校验。

###### 5.7.5.5.3 交互逻辑

- 入口：关联订户 + 勾选恰好 1 条未开始/进行中验收单；否则禁用或提示勾选。打开前设为当前工作验收单。  
- 第 1 步无「导入模板 / 下载模板」。  
- 第 3 步须全部行通过才可进入第 4 步；提示「全部校验成功，才可收货」。  
- 第 4 步展示总种数 / 总套数 / 总册数 / 总码洋 / 总实洋后确认。  
...
- 数据错误、未匹配、验收单不匹配、套数非法同预验收。  
- **不**做已映射字段与订单行一致性比对。  
- 0＜收货套数≤待收且已匹配且头属性一致 → 通过。  

**确认验收**

- 验收单须仍为未开始/进行中；否则整单失败：「验收失败，请重新验收或联系管理员」。  
- 逐行：发货套数 ≤ **实时**待收则可写；否则该行失败。允许部分成功。  
- 文案：「已成功验收 X 条数据，失败 X 条数据。」失败可下载异常数据（订单行号、失败原因）。  
- 开启自动条码时展示分配区间；有空条码提示「注意：本次收货有空条码」。  
- 有成功写入且原为未开始 → 验收单变为进行中。  
...
在已设为当前工作的验收单下，按条件检索订单行，选中后打开合并处置弹窗，以页签完成收货、换货、退货；写回本验收单按种明细与行上发/收/换/退统计。
...
4. 在「收货 / 换货 / 退货」页签中分别确认；待收货套数仍大于 0 时可继续操作，等于 0 时弹窗自动关闭。
...
1. 点击行或行首单选 → 选中并自动打开处置弹窗（默认「收货」页签）。
...
合并弹窗以页签完成收货、换货、退货；各页签独立确认。固定弹窗高度，切换页签高度不变。

###### 5.8.5.4.2 页面要素

- 标题：「逐条收货/换货/退货」+ 订单行号（orderLine）
- 统计条：发订 / 已收 / 已换 / 已退 / **待收** / 订单行备注
- 页签：收货、换货、退货
- 底栏：关闭 + 当前页签确认（收货页可含预览）

**收货 · 纸质书**：ISBN、作者、正题名、定价、币种、实洋、套内册数、收货套数、条码初始号、收货备注  

**收货 · 视听**：ISBN、ISRC、商品条码、目录号、载体、版本/格式、题名、著者、码洋、币种、实洋、彩胶颜色、厂牌、限量编号、套内件数、收货套数、收货备注  

**换货**：换货数量（exchangeQty）、换货原因（exchangeReason）、换货备注  

**退货**：退货数量（returnQty）、退货原因（returnReason）、退货备注  

###### 5.8.5.4.3 交互逻辑

1. 打开默认「收货」页签；切换页签保留未提交草稿。
2. 每次只提交当前确认的页签；成功提示「收货成功」/「换货成功」/「退货成功」。
3. 成功后刷新统计与行数据；待收货套数 &gt; 0 则保持打开并清空已提交页签；= 0 则自动关闭。
4. 点遮罩或关闭不提交草稿。
5. 纸质且需分配条码：「预览」展示预分配/未分配，约 3 秒自动关或点「已知晓」；确认收货前展示分配结果，约 3 秒自动「已知晓」完成确认（可有空条码提示）。
...
| 字段 | 必填 | 规则 |
|------|------|------|
| 收货套数 / 换货数量 / 退货数量 | 是（对应操作） | 正数，且 ≤ 提交时刻待收货套数 |
| 纸质：正题名、定价、实洋、套内册数 | 是 | 开启自动分配条码时条码初始号必填 |
| 视听：载体、题名、码洋、实洋、套内件数 | 是 | — |
| 换货原因 | 换货时是 | 换货、残缺损 |
| 退货原因 | 退货时是 | 退货、损坏退货 |
| 换/退备注 | 否 | 最多 500 字；纸质收货备注最多 200 字 |
...
|------|------|
| 收货缺必填 | 提示填写对应字段；不关弹窗 |
| 换/退缺数量或原因 | 提示填写数量或选择原因 |
| 套数无效或超待收 | 「操作失败：套数无效」或「超过待收货套数（当前待收 N）」 |
...
- 模块位于 `html/src/modules/order/`、`html/src/modules/acceptance/`
- PRD 面板数据：
