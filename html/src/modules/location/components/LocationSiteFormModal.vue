  ...
  <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" @click.self="emit('close')">
    <div class="bg-white rounded-lg shadow-xl w-full max-w-lg flex flex-col max-h-[90vh]" @click.stop>
      <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 shrink-0">
        <h2 class="text-base font-medium text-gray-800">{{ mode === 'edit' ? '编辑馆址' : '新增馆址' }}</h2>
      ...
      <form class="px-6 py-5 space-y-4 overflow-y-auto" @submit.prevent="submit">
        <div class="flex items-start gap-3">
          <label class="text-sm text-gray-600 w-28 text-right pt-2 shrink-0"><span class="text-red-500">*</span> 馆址编码</label>
          <div class="flex-1">
            <input
              v-model="form.code"
              type="text"
...
function createEmptyForm() {
  return { siteId: '', code: '', name: '', statusEnabled: true, remark: '' };
}
...
function onCodeInput() {
  ...
  delete next.code;
  if (!code) next.code = '请输入馆址编码';
  else if (code.length > SITE_CODE_MAX) next.code = '馆址编码长度不能超过32个字符';
  else if (!isValidLevel4SiteCode(code)) next.code = '馆址编码必须属于四级馆藏地现有编码';
  else if (isDuplicateSiteCode(props.siteRows, code, form.value.siteId || null)) next.code = '馆址编码已存在，请修改';
  errors.value = next;
}
