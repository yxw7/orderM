  ...
  <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" @click.self="emit('close')">
    <div class="bg-white rounded-lg shadow-xl w-full max-w-lg flex flex-col max-h-[90vh]" @click.stop>
      <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 shrink-0">
        <h2 class="text-base font-medium text-gray-800">{{ mode === 'edit' ? '编辑分馆' : '新增分馆' }}</h2>
      ...
      <form class="px-6 py-5 space-y-4 overflow-y-auto" @submit.prevent="submit">
        ...
        <div class="flex items-start gap-3">
          <label class="text-sm text-gray-600 w-28 text-right pt-2 shrink-0"><span class="text-red-500">*</span> 分馆编码</label>
          <div class="flex-1">
            <input
              v-model="form.code"
              type="text"
...
import { getActiveSites, validateBranchForm } from '@/modules/location/data/location-manage';
...
function createEmptyForm() {
  return { branchId: '', siteId: '', code: '', name: '', statusEnabled: true, remark: '' };
}
