...
${prdTable(['操作', '行为'], [
  ...
  ['选用某一模板', '若有表头行号（headerRow）则写入输入框并按该行重新读取表头；按文件列名对齐写入列映射（columnMapping）；标准字段按当前验收单资源类型×语种清洗；恢复是否校验（mustMatchFields，缺省用默认勾选集）；订单行号/收货套数映射列「是否校验」仍强制勾选'],
  ...
  ['「保存模板」（未选）', '表头行号必填，未填提示「请填写表头行号」。须已读表头，否则「请先读取表头后再保存模板」；行号已变更未再读则提示先读取。必填字段未映射则提示（如「请映射「订单行号」字段」）并中止。弹「请输入模板名称」；取消中止；空名提示「请输入模板名称」；重名确认是否覆盖；成功提示「模板已保存」并选中该名称'],
  ['「保存模板」（已选）', '表头行号必填。须已读表头且行号未变更未再读。必填字段未映射则提示并中止。覆盖当前模板的表头行号 + 列映射 + 是否校验列表；提示「模板已更新」（不再弹名称）'],
...
${prdTable(['结果类型', '判定条件', '失败原因（示例）'], [
  ...
  ['不一致', '仅对勾选「是否校验」且已映射的标准字段与订单行不等（收货套数不走本类）。ISBN、ISRC 比对前先去掉发货单映射值中的空格与「-」（订单行侧同样去空格/横线后比较）', '「"{字段}"订单行 …/发货单…」'],
  ['通过', '未命中以上失败；收货套数＞0（含大于待收）', '—']
])}
<ul>
