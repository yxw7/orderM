...
- 位置：订单行列表工具栏（与「生成催缺单」「撤订」「导出订单行」相邻），文案「查重」
- 默认状态：置灰不可用
...
- **重复类型**：单选，默认「**不限**」
  ...
  - **订单查重**：仅检查与其他订单行的重复
  - **馆藏查重**：仅检查与馆藏书目的重复
- **查重字段**：按待查重订单行所属订单的**资源类型**展示可选字段（同资源类型下中文/外文可选字段相同，默认选中随语种分类变化）。顶部「**全部**」复选框联动全选/全不选；任一字段变更时同步「全部」勾选状态。
...
- MARC 页签默认展示第一条书目信息
- 分页切换时保持当前页签状态；打开面板时重置为第 1 页、书目页签，并默认展开第一条书目行（馆藏查重每页条数重置为 5，订单查重重置为 50）
...
- **单件数量**：已分配馆藏地单件数 + 未关联馆藏单件数；若为 0，则书目页签展示灰色「0本」，展开区展示「暂无馆藏分布」
- **有单件**：若单件数量 > 0，则展示馆藏树；未分配馆藏地的单件归入「未关联馆藏」节点
- **无单件**：若单件数量 = 0，则书目仍出现在查重结果中，可查看 MARC、执行关联操作
- **自动关联书目**：
