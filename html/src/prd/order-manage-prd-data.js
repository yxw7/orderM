...
<li>若存在<strong>实际关联书目记录号</strong>（<code>actualBibRecordNos</code>，可多值），在旁显示「<strong>实</strong>」浅蓝徽章</li>
<li>悬停「实」：浮层逐条列出实际关联书目记录号，每条为可点击链接（详见 <strong>5.3.5.10</strong>）</li>
<li>无实际关联书目记录号时不显示「实」标记</li>
</ul>
${prdSection('', '交互')}
<ul>
...
<ul>
<li>从页面<strong>顶部向下滑出</strong>，全宽展示，最大高度为视口高度的 85%</li>
<li>面板结构：标题栏 → 摘要信息区 → 结果内容区（可滚动）→ 分页栏</li>
<li>标题：馆藏查重显示「<strong>馆藏查重结果</strong>」；订单查重显示「<strong>订单查重结果</strong>」</li>
<li>摘要区固定显示：订单行号、查重字段（本次字段中文名）、重复记录数</li>
<li>结果数据较多时，中间内容区出现<strong>垂直滚动条</strong>；标题栏、摘要区、底部分页栏固定</li>
<li>点击遮罩或右上角 × 关闭面板</li>
</ul>
${prdSection('', '馆藏查重结果')}
<p>内容区顶部提供「<strong>书目</strong>」「<strong>MARC信息</strong>」两个页签，默认「书目」。</p>
<p><em>书目页签</em></p>
<ul>
<li>以<strong>表格</strong>展示重复书目，每条书目一行；首列为展开/收起图标，末列为<strong>操作</strong></li>
<li>书目字段随查重订单行的<strong>资源类型 + 语种</strong>动态切换（见下表）</li>
<li>字段为空显示「—」；长文本截断，悬浮查看全文</li>
<li>列较多时<strong>横向滚动</strong>；纵向滚动时表头吸顶</li>
<li>展开后在下方整行展示四层馆藏树：机构 → 馆区 → 分馆 → 馆藏地</li>
<li>馆藏树分支节点可展开/收起；馆藏地以蓝色圆角标签展示复本数（如「1本」）</li>
<li>默认展开当前页第一条书目馆藏树；操作「查看」切换至 MARC 页签展示当前书目</li>
</ul>
...
<li>展示字段随查重订单行<strong>语种</strong>动态过滤（见下表）；不在规则内的 tag 不展示</li>
...
<li><strong>馆藏查重结果</strong>：默认 <strong>5</strong> 条/页，可选 5 / 10 / 20 / 50 条/页</li>
...
<li>分页切换时保持当前页签；打开面板时重置第 1 页、书目页签，默认展开第一条馆藏树（馆藏查重每页 5 条，订单查重每页 50 条）</li>
</ul>
${prdSection('', '业务规则')}
<ul>
...
<div id="overview">
${prdSection('', '功能描述')}
<p>当订单行存在<strong>实际关联书目记录号</strong>（<code>actualBibRecordNos</code>）时，在「书目记录号」列通过「<strong>实</strong>」标记提供快捷入口：悬停浮层内点击某条记录号，以<strong>无遮罩可拖拽浮动弹窗</strong>展示该记录号的 MARC 与单件，支持多窗并排对比。</p>
</div>
<div id="ui">
${prdSection('', '「实」标记悬停浮层')}
<ul>
<li><strong>显示条件</strong>：订单行至少有一条实际关联书目记录号（<code>actualBibRecordNos</code>）</li>
<li><strong>标题</strong>：「实际关联书目记录号」</li>
<li><strong>记录号</strong>：每条为蓝色可点击链接；<strong>不提供「复制全部」</strong></li>
...
<li><strong>使用范围</strong>：仅订单行列表；无实际关联书目记录号时不提供本入口</li>
</ul>
</div>
<div id="exception">
${prdSection('', '异常处理')}
...
export const orderManagePrdData = {
  ...
  tabs: {
    ...
    'order-line': {
      label: '订单行列表',
      itemIds: ['5.3.5.1', '5.3.5.2', '5.3.5.3', '5.3.5.4', '5.3.5.5', '5.3.5.6', '5.3.5.7', '5.3.5.8', '5.3.5.9', '5.3.5.10']
    }
  },
  items: [
