...
|--------|------------|--------|--------|----------|
| v2.89  | 2026-09-09 | 产品部 | —      | 对照前端同步订户新增/编辑三页签：业务范围 / 查重显示 / 所属组织（机构·所属馆·馆藏地）（5.9） |
| v2.88  | 2026-09-08 | 产品部 | —      | 对照前端完整同步查重 PRD（5.3.5.4–5.3.5.6）：馆藏重复本馆/全市、书目题名去副题名、单件列序与组织优先排序；修正 5.3.5.5 排序表述 |
| v2.87  | 2026-09-08 | 产品部 | —      | 馆藏重复本馆/全市计数；书目单件数量列位；单件列序与借阅次数；树一～三级与单件按订户组织优先排序（5.3.5.6） |
| v2.86  | 2026-09-08 | 产品部 | —      | 对照前端完整同步订户列表 PRD 与抽屉（5.9：四许可/所属组织机构/详情页签；废弃配置入口折叠展示） |
| v2.85  | 2026-09-08 | 产品部 | —      | 订户编辑四许可文案：归属机构组织 → 所属组织机构（5.9） |
| v2.84  | 2026-09-08 | 产品部 | —      | 订户新增/编辑内嵌四许可；所属组织机构（机构/馆区/分馆）；列表列改所属机构；详情基本信息/权限范围（5.9） |
...
| 馆藏查重 | 比对馆藏书目库是否存在重复；列表按本馆/全市单件数展示有无 |
| 订单查重 | 比对当前订户可见范围内其他订单行是否存在重复 |
| 本馆单件数 | 当前用户关联订户所属机构合集（去重）内的馆藏单件数 |
| 全市单件数 | 命中书目下全市范围的馆藏单件数 |
...
└── 订单行 (OrderLine) []
    ...
    ├── 行状态、验收状态、结算状态
    └── 查重结果（holdingDuplicate / holdingLocalItemCount / holdingCityItemCount / orderDuplicate）
```
...
| 8 | 非连续出版物订单 | 订单行列表-查重 | P0 | 馆藏查重按订户所属组织合并范围；列表馆藏重复「有/无（本馆/全市）」；结果单件全市展示；树一～三级与单件按关联订户机构→馆区→分馆优先排序 |
...
| 31 | 订户管理 | 订户列表-筛选与表格 | P0 | 多条件检索（含所属机构）、创建日期排序、行操作、查看馆员 |
| 32 | 订户管理 | 新增/编辑订户 | P0 | 完整表单：基本信息 + 三页签（业务范围/查重显示/所属组织）；无独立权限配置入口 |
| 33 | 订户管理 | 配置弹窗 | — | 【已废弃】已合并至新增/编辑 |
| 34 | 订户管理 | 查看/启停/删除 | P0 | 详情只读页签（基本信息 / 权限范围）；停用/启用/删除确认 |
...
1. 检索目标订单行。
2. 对待发订行执行查重（批量最多 50 条，或单行查重）：查看馆藏重复「有/无（本馆/全市）」与订单重复「有/无」；若本轮含馆藏查重且书目记录号（bibRecordNo）为空、馆藏结果非空，则自动写入结果第一条书目记录号；全市单件数非 0 时可打开查重结果面板。
...
- **馆藏重复 / 订单重复**筛选规则：
  - **全部**：不过滤
  - **有**：仅显示对应标识为「有」的订单行（`holdingDuplicate` / `orderDuplicate` 为 `true`）；馆藏重复列展示可为「有（本馆/全市）」，筛选只认布尔有无，不解析括号数字
  - **无**：仅显示对应标识为「无」的订单行（值为 `false`）；**未查重**（空白）不匹配「有」或「无」；馆藏重复列可为「无（本馆/全市）」或「无（0/0）」
...
点击查重入口后弹出查重配置弹窗，用户选择重复类型与查重字段后执行查重。**仅馆藏查重**使用当前馆员关联订户在订户列表「编辑 → 所属组织」中的合并结果（见 **5.9**）检索有无馆藏单件；**订单查重不使用查重范围**，仍按当前馆员关联订户下的订单行比对。弹窗内不再选择范围。
...
- **查重范围**：弹窗内**不展示**。当重复类型为「不限」或「馆藏查重」时，提交携带按馆员关联订户顺序合并的所属分馆编码（branchCodes）（由各订户「所属组织」解析；馆藏地 collectionCodes 本版为空）（规则见 **5.9.5.3**），用于馆藏单件检索；皆空表示不限。「订单查重」不带入范围。关联订户合并后的**机构 / 所属馆**有序合集同时作为查重结果「单件」页签树一～三级与单件列表的**优先排序**依据（见 5.3.5.6）。
...
- 点击「确定」：
  ...
  - 查重完成后**自动关闭弹窗**，刷新列表馆藏重复（holdingDuplicate、holdingLocalItemCount、holdingCityItemCount）、订单重复（orderDuplicate）标识列；若本轮含馆藏查重，则按 5.3.5.6.4 自动关联规则同步更新书目记录号（bibRecordNo）
...
- 其他字段比对时忽略大小写
- 馆藏查重范围来自订户「编辑 → 所属组织」合并，**仅馆藏查重**用于检索有无馆藏单件；合并结果为空视为不限
- 订单查重**不使用**查重范围，按当前馆员关联订户下的订单行比对
- 关联订户合并后的**机构 / 所属馆**有序合集作为查重结果「单件」页签树一～三级与单件列表的优先排序依据（见 5.3.5.6）；馆藏检索仍用分馆编码范围，本馆计数仍只按所属机构合集
- 馆藏查重完成后写回：本馆单件数（holdingLocalItemCount）、全市单件数（holdingCityItemCount）、馆藏重复（holdingDuplicate，本馆 > 0 为 true）
...
查重完成后，在订单行列表展示重复标识；用户可点击馆藏重复文字链（全市单件数非 0）或订单重复「有」查看详细查重结果。

**列表重复标识列**

列表包含两列：**馆藏重复**（holdingDuplicate + holdingLocalItemCount / holdingCityItemCount）、**订单重复**（orderDuplicate）。
...
|------|------|--------------|
| 未查重 | 空白 | 否 |
| 本馆单件数 > 0 | 蓝色文字链「有（本馆/全市）」 | 是 |
| 本馆 = 0 且全市 > 0 | 蓝色文字链「无（本馆/全市）」 | 是 |
| 本馆 = 0 且全市 = 0 | 纯文本「无（0/0）」 | 否 |

- **本馆单件数**：当前用户关联订户的**所属机构合集（去重）**内单件数合计（各命中书目本馆数求和）
- **全市单件数**：命中书目全市单件数合计
- **有 / 无**：仅按本馆单件数判定（>0 有，=0 无）；废除「匹配到书目即有」
- **筛选**：仍全部 / 有 / 无，只认 `holdingDuplicate`，不解析括号数字
...
| 维度 | 本馆单件数 > 0 | 本馆 = 0 且全市 > 0 | 本馆 = 0 且全市 = 0 |
|------|----------------|---------------------|---------------------|
| 列表「馆藏重复」 | **有（本馆/全市）** 可点 | **无（本馆/全市）** 可点 | **无（0/0）** 不可点 |
| 是否出现在书目列表 | 是（有命中书目时） | 是 | 是（仅书目命中且全市为 0） |
| 书目页签「单件数量（本馆/全市）」 | `本馆/全市`（如 10/50） | `0/全市` | `0/0` |
| 单件页签 | 全市单件 + 馆藏树 | 全市单件 + 馆藏树 | 「暂无馆藏分布」/「暂无单件信息」 |
| MARC / 关联 | 可用 | 可用 | 可用 |

**单件数量（书目行）**：`localItemCount/cityItemCount`；全市为该书目全部关联单件（已分配馆藏地 + 未关联）。「未关联馆藏地」挂载规则不变：所属馆与所属馆藏地皆空 → **一级根节点**「未关联馆藏地」；所属馆有值且所属馆藏地为空 → 挂在匹配到的**所属馆节点下**的四级叶子；所属馆无法匹配时并入一级根。

*书目页签*

- 表格列顺序：**单选** → **书目记录号** → **单件数量（本馆/全市）**（单元格文案为 `本馆/全市`，如 `10/50`） → 其余书目字段 → **操作**
- 单选：每行一个 radio；打开面板或书目分页切换后默认选中**当前页第一条**；MARC / 单件始终展示当前单选书目
- 书目字段随资源类型与语种动态切换；统一列名「**题名**」（不再用「正题名」）；题名后固定增加：**分卷号**、**分卷名**（空值「—」）；**不展示副题名**
  - 纸质书·中文：书目记录号、题名、分卷号、分卷名、ISBN、作者、出版社、出版年、版本
  - 纸质书·外文：书目记录号、题名、分卷号、分卷名、ISBN、责任者、出版社、出版日期、语种
  - 视听资料·中文：书目记录号、题名、分卷号、分卷名、载体、ISBN/ISRC、出版社、版本/格式、著者
  - 视听资料·外文：书目记录号、ISRC、题名、分卷号、分卷名、载体、商品条码、目录号、出版方
...
- 进入页签或切换单选书目时未选叶子，右侧显示该书目**全市**单件
- 不显示底部分页栏；列顺序：**馆藏状态**、条码号、所属馆、所属馆藏地、**借阅次数**、卷册描述、单件价格、借阅类型、装帧、索书号、排架标引分类、排架标引（删除所在馆、所在馆藏地、登到时间）
- **借阅次数**（loanCount）：外部接口提供；数据中台约每月更新；空值「—」
- **优先合集**：当前用户关联订户所属组织（机构 / 所属馆）按订户顺序合并去重保序
- **馆藏树排序**：一～三级按优先合集排序（同父下比较，不跨父搬迁）；取消「首都图书馆」特殊置顶；四级不重排；一级「未关联馆藏地」仍排末尾
- **单件列表排序**：按同一优先合集（分馆 → 馆区 → 机构回落）；同键保持原相对序；未命中排后
...
- 馆藏重复：全市单件数 ≠ 0 时可点击打开统一查重结果面板（默认书目页签）；`无（0/0）` 与未查重空白不响应
- 订单重复：标识为「有」时可点击（默认订单页签）；「无」或空白不响应
...
- **订单查重**比对范围：当前登录馆员**关联订户**下的其他订单行（不含当前行自身）；**不使用**查重范围（所属组织）
- **馆藏查重**比对范围：按订户「编辑 → 所属组织」合并后的分馆编码范围，检索有无馆藏单件数据（见 5.3.5.5）；皆空则不限
- **馆藏重复判定**：本馆单件数 > 0 为「有」，否则已查重为「无」；与是否仅匹配到书目无关
- **本馆 / 全市计数**：书目行写 `localItemCount` / `cityItemCount`；订单行写各书目求和后的 `holdingLocalItemCount` / `holdingCityItemCount`；展示侧单件为全市，本馆计数按所属机构合集，不按馆区/分馆进一步收窄
- **自动关联书目**：
...
- **单件 / 馆藏树排序**：按关联订户所属组织优先合集（机构→所属馆，去重保序）；合集空则不额外重排；取消首都图书馆特殊置顶
...
- 馆藏重复全市为 0 或未查重、订单重复为「无」或未查重时，不打开结果面板
- 书目 / 订单页签无数据时展示「暂无查重结果」，且**不展示**底部分页栏
- 单件页签无馆藏分布或全市单件数量为 0：左侧「暂无馆藏分布」、右侧「暂无单件信息」
...
- 馆藏查重：按订户「编辑 → 所属组织」合并后的分馆编码范围检索馆藏单件（见 5.9）；皆空则不限。
...
维护订户主数据：列表检索与启停删除；**新增/编辑**在同一弹窗中维护基本信息（名称、备注）与三个横向页签——**业务范围**、**查重显示**、**所属组织**。馆藏查重提交时按当前馆员关联订户顺序合并各订户「所属组织」对应分馆编码（见 5.3.5.5）。本版无独立「权限配置」入口、无所属分馆排序 UI、无数据归属。
...
2. 点击「新增订户」填写完整表单（基本信息 + 三页签）并提交。
3. 点击订户名称查看只读详情（基本信息 / 权限范围）。
4. 点击「编辑」维护完整表单并保存。
...
- 各已填条件为 AND 组合
- 资源类型、预算按订户已配置业务范围字段匹配；所属机构按展示名模糊匹配
...
- 工具栏：「新增订户」
- 表格列：序号、订户名称、所属机构、资源类型、预算名称、订户状态、馆员列表、创建日期、操作
- 订户名称：蓝色链接，点开查看详情
- 订户状态：使用中（绿色）/ 已停用（红色）
- 馆员列表：有数据且可查看时蓝色「查看」；否则灰色「查看」不可点
- 操作列：编辑；使用中另有停用、删除；已停用另有启用、删除（无独立「权限配置」）
- 创建日期列头可切换升/降序（默认降序）
- 分页：默认 50 条/页，可选 10 / 20 / 50
...
- 列表「所属机构 / 资源类型 / 预算名称」取自该订户编辑保存字段；所属机构未填时展示「—」
- 新增订户默认状态为使用中；基本信息与三页签字段在新增/编辑弹窗中一次维护
...
新增与编辑共用同一弹窗：上半基本信息（订户名称、备注）；备注下方三个横向页签——**业务范围**、**查重显示**、**所属组织**。一次提交保存全部字段。本版无独立「权限配置」入口、无「添加操作许可」、无数据归属。

###### 5.9.5.3.2 页面要素

- 标题：新增订户 / 编辑订户；内容区可纵向滚动；底栏取消 / 提交（新增）或 取消 / 保存（编辑）
- 上半：订户名称、备注
- 下半页签（默认「业务范围」）：业务范围 / 查重显示 / 所属组织
...
- 打开时带入当前订户已存数据（编辑）或空表（新增）；默认选中「业务范围」页签
- 变更所属机构 → 清空所属馆、所属馆藏地；变更所属馆 → 清空所属馆藏地
- 变更查重显示「所属分馆」后，剔除不再属于所选分馆下级的「所属馆藏地」选项
- 校验失败时字段下展示错误文案，不关闭弹窗，并自动切到首个失败字段所在页签
- 提交成功后关闭弹窗并刷新列表

###### 5.9.5.3.4 业务规则

- 所属组织：所属机构必填；所属馆、所属馆藏地可空
- 查重显示两字段均可空
- 停用节点：编辑时已选项仍可展示
- **馆员多订户合并（供馆藏查重提交）**：按馆员关联订户的**先后顺序**合并各订户「所属组织」解析出的分馆编码（首次出现的编码保留）；合并结果皆空则馆藏查重不限范围（见 5.3.5.5）
- 订单查重不使用本范围
...
##### 5.9.5.4 【已废弃】功能模块名称：配置弹窗

原独立「配置」弹窗已合并至 **5.9.5.3**；列表不再提供「配置」入口。业务范围、查重显示、所属组织在新增/编辑弹窗页签内一次维护。**禁止重用本编号。**

##### 5.9.5.5 功能模块名称：查看订户详情

###### 5.9.5.5.1 功能描述

只读展示订户基本信息与权限范围（含业务、查重显示、所属组织相关字段）。

###### 5.9.5.5.2 页面要素

- 标题：查看订户；页签：基本信息 / 权限范围
- 基本信息：订户名称、订户状态、创建日期、备注
- 权限范围：资源类型、预算管理、条码号类型、查重·分馆、查重·馆藏地、所属机构、所属馆、所属馆藏地（空则「—」）
- 底部：关闭
...
无（当前为前端 Mock）。馆藏查重提交侧读取按馆员关联订户「所属组织」合并后的所属分馆编码（branchCodes）（见 5.3.5.5、5.9.5.3）。查重显示页签字段（displayBranchCodes / displayCollectionCodes）本版用于订户配置展示，与查重提交合并规则独立。
