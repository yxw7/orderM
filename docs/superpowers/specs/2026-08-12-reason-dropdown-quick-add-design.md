...
**关联**：设置页 `ReasonParamsView.vue`；store `reason-params.js`；现有各业务原因 `<select>`
...
自定义下拉：选项列表 + 底部「＋ 添加原因」+ 内联单字段输入。按 `reasonType` 读写 store，替换各处原生原因 `<select>`。
...
1. 点击触发器展开选项列表；底部固定「＋ 添加原因」
2. 点「＋ 添加原因」：收起选项列表，展开内联输入区（原因内容 + 取消 / 保存）
3. 保存成功：关闭输入区、刷新选项、**自动选中**新原因、收起下拉
4. 取消或点击外侧关闭：丢弃未保存输入，回到普通下拉状态
5. `disabled` 时不可展开、不可添加
...
1. 四类原因各走一遍：快捷添加 → 自动选中 → 业务确定成功；设置页可见且为使用中
2. 与已有使用中 / 停用同名文案冲突时拒绝新增
3. 空列表仅靠快捷添加完成一次撤订或退货
4. 取消添加不改变原选中值；关闭业务弹窗再开，输入区已清空
5. 抽查撤订弹窗、处置面板双下拉、发货单行内下拉，交互一致且不被裁切

---

## 8. PRD / 文档

实现时同步相关 PRD 文案（如撤订原因「来源于设置」补充「业务下拉支持快捷添加并写入同一参数库」），范围以实际改动的模块条目为准。
